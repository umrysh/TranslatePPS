...
# From http://l.umrysh.com/OyV
def rename(dir, pattern):
    for pathAndFilename in glob.iglob(os.path.join(dir, pattern)):
        title, ext = os.path.splitext(os.path.basename(pathAndFilename))
        os.rename(pathAndFilename, 
                  os.path.join(dir, title + ".ppt"))

### Start PyODConverter Code ###
...
class DocumentConverter:
    ...
    def __init__(self, port=DEFAULT_OPENOFFICE_PORT):
        ...
        try:
            ...
            raise DocumentConversionException("failed to connect to OpenOffice.org on port %s\n\nRun: soffice \"--accept=socket,port=2002;urp;\"" % port)
...
### End PyODConverter Code ###

if __name__ == "__main__":   
    if len(sys.argv) < 2:
        print("USAGE: python %s <input-folder>" % sys.argv[0])
        exit(255)
    if not os.path.isdir(sys.argv[1]):
        print("no such input folder: %s" % sys.argv[1])
        exit(1)

    # Look in folder for pps, remame to ppt
    rename(r'%s' % sys.argv[1], r'*.pps')

	# Convert all ppt to odp

    try:
        converter = DocumentConverter() 
        for name in [item for item in os.listdir(sys.argv[1]) if os.path.isfile(os.path.join(sys.argv[1], item)) and os.path.splitext(item)[1].lower() == ".ppt"]: 

            converter.convert("%s/%s" % (sys.argv[1],name), "%s/%s.odp" % (sys.argv[1],os.path.splitext(name)[0]))

            # Pull in the content.xml
            zip_in = zipfile.ZipFile("%s/%s.odp" % (sys.argv[1],os.path.splitext(name)[0]), mode='a')
            temp = zip_in.open('content.xml')
            temp = codecs.EncodedFile(temp, 'utf-8', 'utf-8').read()
